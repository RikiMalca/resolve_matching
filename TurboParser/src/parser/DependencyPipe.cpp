...
void DependencyPipe::MakeParts(Instance *instance,
  ...
  if (train_pruner_) {
    ...
    // Make global parts.
    MakePartsGlobal(instance, parts, gold_outputs, ilansChanges);
    dependency_parts->BuildOffsets();
  }
}
...
void DependencyPipe::MakePartsGrandSiblings(Instance *instance,
  ...
  for (int g = 0; g < sentence_length; ++g) {
    ...
    for (int h = hLimit; h < sentence_length; ++h) {
      if (g == h) continue;
      ...
      for (int m = mLimit; m < sentence_length; ++m) {
        int r1 = -1;
        if (m != h) {
          r1 = dependency_parts->FindArc(h, m);
          if (r1 < 0) continue;
        ...
        int sLimit = sentence_length;
        if (ilansChanges) {
        	sLimit = sentence_length - 1;
        }
        for (int s = m+1; s <= sLimit; ++s) {
          int r2 = -1;
          if (s < sentence_length) {
            r2 = dependency_parts->FindArc(h, s);
            if (r2 < 0) continue;
  ...
  for (int h = 0; h < sentence_length; ++h) {
    ...
    for (int m = mLimit; m < sentence_length; ++m) {
      ...
      for (int s = m+1; s < sentence_length; ++s) {
        ...
        int tLimit = sentence_length;
        if (ilansChanges) {
        	tLimit = sentence_length - 1;
        }
        for (int t = s+1; t <= tLimit; ++t) {
          int r3 = -1;
    ...
    for (int m = mLimitUp; m >= mLimitDown; --m) {
      int r1 = -1;
      if (m != h) {
        r1 = dependency_parts->FindArc(h, m);
        if (r1 < 0) continue;
