	...
	for (int j = 0; j < vSubTree.size(); j++) {
		...
		for (int root2 = 1; root2 < sentenceSize; root2++) {
			...
			for (int node2_index = 0; node2_index < (*subTrees)[root2].size(); node2_index++) {
				...
				if (r2 < 0) continue;
				(*edge2LostEdges)[r1].push_back(r2); // now r1 makes a cycle with r2.
 ...
 */
{
	...
	//gettimeofday(&tp, NULL);
	//LOG(INFO) << "geedy : initialization : " << tp.tv_usec- time_begin;
	//gettimeofday(&tp, NULL);
	//time_begin = tp.tv_usec;
	sort(EdgesMap.begin(), EdgesMap.end(), compareByScore);
	//gettimeofday(&tp, NULL);
	//LOG(INFO) << "geedy : sorting1 : " << tp.tv_usec- time_begin;
	//gettimeofday(&tp, NULL);
	//time_begin = tp.tv_usec;
	...
	//gettimeofday(&tp, NULL);
	//LOG(INFO) << "geedy : main_loop : " << tp.tv_usec- time_begin << endl;
	//time_begin = tp.tv_usec;
